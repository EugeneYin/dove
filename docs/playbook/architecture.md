...
顶栏出现「新版本 · 刷新」，由用户点。页面注册 Service Worker 后还必须主动检查 `registration.waiting`；
`updatefound` 不会为已经进入 waiting 的 Worker 重放，仅监听事件会让再次打开的普通窗口永久停在旧版。
在该检查首次发布前已经卡住的客户端，应关闭全部相关标签页/PWA 后重新打开；仍未恢复时，先注销该站点的
Service Worker 并硬刷新，不要先清除站点数据，以免删除本地保存的最近阅读和单词本。
...
真机上没有 devtools。iPad、Android Pad 上出问题时，既看不到 console，也很难复现，
唯一能拿到的信息就是用户的一句「打不开」。诊断层的目标是把这句话换成一份可复制的现场。

分成两半，分界线是**模块能不能跑起来**：

| 位置 | 何时工作 | 负责 |
|---|---|---|
| `index.html` 内联脚本（ES5） | 总是 | 收日志、接管 `console.warn/error`、捕获 `error` 与 `unhandledrejection`、兜底渲染 |
| `src/diag.ts` | 模块加载成功后 | 完整面板：环境、能力自检、运行时状态、日志、复制、重置 |

日志收集必须在内联脚本里，理由与那段兜底诊断相同：**模块没起来的那些故障，恰恰是最
需要日志的时候**。`src/diag.ts` 只是读 `window.__dove` 里已经攒下的内容，并把
`store.open` 换成真正的面板；若它自己没加载成功，顶栏的「诊断」按钮仍能把日志倒在页面上。

日志滚动写入 `localStorage`（上限 400 条）。这一条不是锦上添花：崩溃、白屏后自动刷新、
或者用户自己重开应用之后，内存里的日志已经没了，面板顶部那份「上次会话」才是现场。

面板里有三项是别处拿不到的：

- **缓存明细** — 页面通过 `MessageChannel` 向 SW 发 `{type:"diag"}`，SW 逐个核对
  EXTRAS 是否在缓存里，把**缺失文件的完整清单**回传。「装上了但某个功能时好时坏」
  几乎都是某个文件没进缓存，而进度条只报了个失败计数，看不出缺的是词典还是语言包。
- **画布墨迹** — 在当前画布上抽样 40×40 个点判断有没有内容。移动端超过画布面积上限时
  `getContext` 与 `render` 都照常成功，只是画出来全是空白且不报错；这一行是区分
  「没渲染」与「渲染了但看不见」的唯一证据。
- **预缓存失败原因** — SW 的进度消息带上失败 URL 与原因（`HTTP 404`、网络错误等），
  由页面写进日志。
