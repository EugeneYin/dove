...
- 关键验收优先使用与 RD 不同的 Agent 实例，条件允许时使用不同模型，降低同源盲点；
...
模型能力画像：Opus 5 一类擅长反复验证、代码审查和风险发现的模型适合独立 QA；GPT-5.6 Sol
`high` 适合需要深入执行测试工具和代码级定位的回归；Fable 5 适合超长证据链或大型仓库级审计。
模型自评不构成证据，最终结论仍必须来自可复现测试和可检查附件。
