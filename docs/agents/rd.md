...
复杂 Coding、终端工具、多文件修改和长上下文调试优先使用 GPT-5.6 Sol `high` 一类前沿 Coding
模型。Opus 5 适合需要持续迭代、自我验证、前端或细致代码审查的实现；Fable 5 适合极长、跨仓库或
高度复杂的研发任务。应根据任务做最小充分选择，而不是所有 Coding 都使用最高成本模型。

模型拥有 shell、Apply Patch、MCP 或计算机操作能力，不代表获得了新的权限。所有外部写操作、
生产变更和破坏性命令仍受 PMO 与用户授权约束。
