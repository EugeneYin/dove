...
const paths = ["e2e/test-cases.json", "e2e/device-test-cases.json"];
...
for (const path of paths) {
  ...
  const sourceIds = [...source.matchAll(/test\.step\(["'`]([A-Z]+-\d{3})\b/g)].map(
    (match) => match[1],
  );
  ...
  if (duplicateCatalogIds.length)
    problems.push(`目录中存在重复编号: ${duplicateCatalogIds.join(", ")}`);
  if (duplicateSourceIds.length)
    problems.push(`测试中存在重复编号: ${duplicateSourceIds.join(", ")}`);
